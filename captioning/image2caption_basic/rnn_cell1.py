...
class GRUCell(RNNCell):
  ...
  def __call__(self, init_state, inputs, state, scope=None):
    """Gated recurrent unit (GRU) with nunits cells."""
    with vs.variable_scope(scope or type(self).__name__):  # "GRUCell"
      with vs.variable_scope("Gates"):  # Reset gate and update gate.
        # We start with bias of 1.0 to not reset and not update.
        r, u = array_ops.split(1, 2, linear([init_state, inputs, state],
                                            2 * self._num_units, True, 1.0))
        r, u = sigmoid(r), sigmoid(u)
      with vs.variable_scope("Candidate"):
        c = tanh(linear([init_state, inputs, r * state], self._num_units, True))
      new_h = u * state + (1 - u) * c
    return new_h, new_h
...
class MultiRNNCell(RNNCell):
  ...
  def __call__(self, initial_state, inputs, state, scope=None):
    ...
    with vs.variable_scope(scope or type(self).__name__):  # "MultiRNNCell"
      ...
      for i, cell in enumerate(self._cells):
        with vs.variable_scope("Cell%d" % i):
          ...
          cur_inp, new_state = cell(initial_state, cur_inp, cur_state)
