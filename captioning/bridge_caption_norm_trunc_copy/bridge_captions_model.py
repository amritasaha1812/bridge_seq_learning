...
class BridgeCaptionsModel():
  ...
  def __init__(self, max_seq_length, embedding_size, batch_size, rnn_size, e_vocab_size,
          f_vocab_size, image_size, lmbda, lmbda_cov, random_seed, activation=sigmoid,
      ...
      self.embedding_size_squared = embedding_size*embedding_size
  ...
  def compute_correlation(self, x, y) :
    ...
    ss_dr1 = tf.reduce_sum(tf.mul(x_centered, x_centered), 0)
    ss_dr2 = tf.reduce_sum(tf.mul(y_centered, y_centered), 0)
    corr_dr1 = tf.sqrt(ss_dr1)
    corr_dr2 = tf.sqrt(ss_dr2)

    corr_dr = tf.mul(corr_dr1, corr_dr2)  

    corr = tf.div(corr_nr, corr_dr)

    x_standardized = tf.div(x_centered, corr_dr1)
    y_standardized = tf.div(y_centered, corr_dr2)
    cov_x = (tf.reduce_sum(tf.abs(tf.matmul(x_standardized, x_standardized, transpose_a=True)))-self.embedding_size)/(self.embedding_size_squared-self.embedding_size)
    cov_y = (tf.reduce_sum(tf.abs(tf.matmul(y_standardized, y_standardized, transpose_a=True)))-self.embedding_size)/(self.embedding_size_squared-self.embedding_size)
    cov = ( cov_x + cov_y )/2.0	
    #return x_centered, y_centered, corr_nr, corr_dr1, corr_dr2, corr_dr, tf.reduce_sum(corr)
    return corr, tf.reduce_sum(corr), cov
  ...
  def loss_e_i(self, e_sequences, e_seq_masks, images, lmbda, lmbda_cov, weights) :
    ...
    # ==================================================================================================================
    """
    corr_vec, l_ei_corr, cov = self.compute_correlation(h_e, h_i)

    loss = tf.reduce_sum(tf.sub(weights, corr_vec)) + lmbda_cov * cov 
    ...
    return l_ei_corr, loss, h_e, h_i, corr_vec, cov
